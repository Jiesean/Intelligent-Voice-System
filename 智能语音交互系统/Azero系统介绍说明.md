...
声纹确认在智能语音系统中一般用于进行用户的身份确认，以确定是否对该用户的请求和命令进行后续响应

#### 基本流程

![声纹确认流程](./voiceprint.png)

声纹确认一般是以用户为单位的，所以声纹模型库的训练不像唤醒一样，可以使用现有大批量的语料提前训练，声纹一般是需要在产品的使用初始阶段，收集用户的语音，然后通过这些语料来训练声纹模型库。除了声纹模型库的训练方法有所差异，其他的流程和唤醒是基本相同的，都是通过模型库的关键词检出和匹配度得分来和设定的阈值进行比较，以决定对语音接受或者拒绝

#### 评价指标

1. 错误拒绝率，该指标表示目标用户说了多少次，被错误拒绝的次数的占比
2. 错误接受率，该指标表示非目标用户说了多少次，被错误接受的次数的占比
3. 功耗，很多声纹确认模型一般跑在移动设备上，因此我们需要考虑模型的复杂度，尽可能的优化声纹确认的功耗
4. 时延，该指标表示声纹算法带来的整个语音链条上的延时
...
2. 文本无关
   ...
   - 需要考虑如何无感的手机训练语料和用户确认的过程，尽可能的避免打扰用户

3. 防假冒

   声纹一般用于用户身份的鉴别，以提高语音系统、产品的安全性，因为涉及到安全问题，那么声纹可能存在一些安全的隐患，其中包括：

   - 模仿，恶意模仿目标用户的声色以欺骗声纹算法
   - 录音，录取目标用户的声音以欺骗声纹算法
   - 语音合成，使用语音合成的方法来欺骗声纹算法

   在现有的技术局限下，上述的方式是很难完全杜绝的，因此在产品上考虑声纹系统作为安全校验的环节时，需要考虑其弱安全性，需要通过其他的方式进行充分保证，另外需要通过灵活的动态声音片段来进行声纹校验，以避免上述的欺骗行为的发生

4. 鲁棒性

   - 环境，环境是安静还是噪音，会很大程度的影响到声纹确认的结果
   - 设备，训练时使用的设备和检测时使用的设备的不同，也可能会影响到声纹确认的效果
   - 说话人状态，说话人在不同时间，不同状态下声音的音色也可能是完全不同的，这也会影响到声纹确认的结果
...
在智能语音系统的交互场景中，语音系统通常扮演着类似语音助手的角色，交互场景的发起者是用户，语音系统通过前面的各种技术，将用户的语音转换成程序可以理解的意图，然后我们对意图进行处理后，我们需要对用户的意图进行相应，相应需要有一定的拟人性，那么很多场景下，就需要我们的智能语音系统可以进行人声的合成和播放，将文字转换成语音流的过程，我们称之为 TTS ,text to speech

### 基本流程

![tts基本流程](./tts.png)

 TTS的生成目前主要有两种，一种是基于片段拼接，就好比活字印刷书，我们提前录制很多很多的元单元的语音频段，前面经过文本分析韵律停顿，声学参数预测等过程，之后从已有的音频库里面选择符合要求的语音片段进行拼接，最终形成我们想要的tts流；还有一种是基于参数合成的，所有的参数都事先经过语音训练好的，经过前面同样的前置处理，得到声学参数之后，他通过一个声码器来将参数转换成语音。
